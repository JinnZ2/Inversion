...
Vendored into the Inversion repository from:
    https://github.com/JinnZ2/thermodynamic-accountability-framework
    (top-level ``resilience_stack.py``)

The upstream module is the authoritative source. This copy is held here
so that the Inversion corpus can run the assessment without a cross-repo
install step. Keep in sync manually if the upstream diverges.
...
class AbsenceType(Enum):
    CONSTRAINT_LITERACY = "constraint_literacy"
    BOUNDED_COMPETENCE = "bounded_competence"
    RELATIONAL_KNOWLEDGE = "relational_knowledge"
    REPAIR_NETWORKS = "repair_networks"
    TRANSLATION_BRIDGES = "translation_bridges"
    SELF_SUFFICIENCY_GRAMMAR = "self_sufficiency_grammar"
...
class AbsenceSignature:
    ...
    description: str
    distortion_signatures: list[str]
...
class BoundedSolution:
    ...
    operating_bounds: dict[str, tuple[float, float]]
...
class ConstraintNavigator:
    ...
    def propose_bounded_solution(
        ...
        return BoundedSolution(
            ...
            valid_conditions=[
                f"dimension {d.name} within [{d.min_safe}, {d.max_safe}] {d.unit}"
                for d in problem.dimensions
            ],
...
class Regulation:
    ...
    stated_intent: str
    scope_defined: bool
    parameters_measurable: bool
    expiration_or_renewal: bool
    exception_handling: bool
    root_cause_linked: bool
...
class RegulatoryScopeAudit:
    ...
    def audit(self, reg: Regulation) -> RegulationAudit:
        ...
        if weaponization_risk >= 4:
            recommendation = (
                "HIGH RISK: likely used for gatekeeping, not safety. Rewrite with "
                "bounded scope, measurable outcome, expiration, exception handling."
            )
        elif weaponization_risk >= 2:
            recommendation = (
                "MODERATE RISK: selective enforcement possible. Tighten scope and add outcome metric."
            )
        else:
...
class ResilienceAssessment:
    ...
    cascade_vulnerability_score: int
    notes: list[str]
...
class ResilienceStack:
    ...
    def assess(
        ...
        if unaccounted and high_risk:
            notes.append(
                "System blind to critical layers AND institutionally gatekeeping the people who "
                "could fill them. Expect accelerated failure under stress."
            )
        ...
        if cascade >= 7:
            notes.append(
                "Cascade vulnerability HIGH. Parallel adaptive systems will emerge whether sanctioned or not."
            )
